...
class AlibabaFliggyLeaseItemUpdateUpdateSkuDto {
    ...
    /**
        发货方式：邮寄/自提/闪送（暂时只支持邮寄）（新增时需要，销售属性不允许修改）
     **/
    public $shipping_type;

    /**
        租赁方案：到期归还、到期归还/续租、到期归还/续租/买断（暂时只支持到期归还）（新增时需要，销售属性不允许修改）
     **/
    public $lease_plan;
    ...
    /**
        买断价格，单位分（仅长租商品，必填）；创单时需写入快照
     **/
    public $buy_out_price;

    /**
        续租方案列表（仅长租商品，必填）；创单时需写入快照
     **/
    public $renewal_plan_list;

    /**
        增值服务列表（仅长租商品）；创单时需写入快照
     **/
    ...
    public function getFirstMonthRentDiscount() : AlibabaFliggyLeaseItemUpdateFirstMonthRentDiscountDto{
        return $this->first_month_rent_discount;
    }

    public function setFirstMonthRentDiscount(AlibabaFliggyLeaseItemUpdateFirstMonthRentDiscountDto $firstMonthRentDiscount){
